...
def crop_recommend():
    import requests
    import pandas as pd
    data=requests.get("https://api.thingspeak.com/channels/2740541/feeds.json?api_key=L0JB8WK7Y53CLXMU&results=2")
...
def crop_predict():
    ...
    if request.method == 'POST':
        ...
        return render_template('crop-result.html', prediction1=crop3[0], prediction2=crop3[1], prediction3=crop3[2],
                                 title=title)
        # else:
...
def yeild_predict():
    ...
    if request.method == 'POST':
        ...
        if out_1[0] > 10000 and int(area) < 10 :
            out_yield=f"{float(out_1[0]/100):.2f}"
        else:
...
def price_predict():
    ...
    if request.method == 'POST':
        ...
        return render_template('price_prediction.html', title=title, p_result=f"{p_result:.2f}")
    return render_template('try_again.html', title=title)
